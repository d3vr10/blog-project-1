...
const envSchema = z.object({
    ...
    S3_ACCESS_KEY_ID: z.string().optional()
        .refine((value) =>
           value || logger.warn("S3_ACCESS_KEY_ID is missing. This disables uploading editor media to remote storage") || true
        ),
    S3_SECRET_ACCESS_KEY: z.string().optional()
        .refine((value) =>
            value || logger.warn("S3_SECRET_ACCESS_KEY is missing. This disables uploading editor media to remote storage") || true
        ),
    S3_MEDIA_BUCKET: z.string().optional().default("media")
        .refine((value) =>
            value || logger.warn('S3_MEDIA_BUCKET is missing. Using "media" bucket by default') || true
        ),
    S3_TMP_BUCKET: z.string().optional().default("tmp")
        .refine((value) =>
            value || logger.warn('S3_TMP_BUCKET is missing. Using "tmp" bucket by default') || true
        ),
}).transform(value => {
